...
Function Request-JitAccess {
    ...
    param(
        ...
        [Parameter(Mandatory=$false, 
            HelpMessage="Override with this ip")]
        [Alias('ip')]
        [string]$IpAddress = ""
    )
    BEGIN {
    ...
    PROCESS {
        ...
        if ($IpAddress -eq "") {
            $IpAddress = (Invoke-WebRequest -Uri https://ifconfig.me/ip | Select-Object Content).Content
            Write-Verbose "Using ip from ifconfig.me: $IpAddress"
        } 
        else {
            Write-Verbose "Using ip from argument: $IpAddress"
        }
        ...
        $Body = "{""requests"":[{""content"":{""virtualMachines"":[{""id"":""$($vmInfo[0])"",""ports"":[{""number"":3389,""duration"":""PT10H"",""allowedSourceAddressPrefix"":""$IpAddress""}]}]},""httpMethod"":""POST"",""name"":""$requestName"",""requestHeaderDetails"":{""commandName"":""Microsoft_Azure_Compute.""},""url"":""$EndPoint""}]}" | ConvertTo-Json
...
Function ConnectTo-VM {
    ...
    if($JitRequest -eq $true) {
        Request-JitAccess -g $rgName -n $name -ip $IpAddress
    }
